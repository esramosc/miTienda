			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="region" title="${message(code: 'region.region.label', default: 'Region')}" />
					
						<g:sortableColumn property="description" title="${message(code: 'region.description.label', default: 'Description')}" />
					
						<g:sortableColumn property="registerDate" title="${message(code: 'region.registerDate.label', default: 'Register Date')}" />
				...
				<g:each in="${regionInstanceList}" status="i" var="regionInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${regionInstance.id}">${fieldValue(bean: regionInstance, field: "region")}</g:link></td>
					
						<td>${fieldValue(bean: regionInstance, field: "description")}</td>
					
						<td><g:formatDate date="${regionInstance.registerDate}" /></td>
